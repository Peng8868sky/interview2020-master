 ...
 *
 * （2）使用方法
 * Synchronized 不需要用户去手动释放锁，当Synchronized代码执行完毕之后，系统会自动让线程释放对锁的占用
 * ReentrantLock 则需要用户手动释放锁，若没有主动释放锁，就有可能导致出现死锁现象。需要lock()和unlock()方法配合try/finally语句块来完成
 *
 * （3）等待是否可中断
 ...
 * （5）锁绑定多个条件Condition
 * Synchronized 没有
 * ReentrantLock 用来实现分组唤醒需要唤醒的线程们，可以精确唤醒，而不像Synchronized 要么随机唤醒一个线程，要么随机唤醒全部线程
 *
 */
...
/**
 * 题目：多线程之间按顺序 调用，实现A-->B-->c 三个线程启动，要求如下：
